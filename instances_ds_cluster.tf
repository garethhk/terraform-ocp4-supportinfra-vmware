...
resource "vsphere_virtual_machine" "bastion_ds_cluster" {
  ...
  clone {
    ...
    customize {
      ...
      dns_server_list = "${compact(concat(var.public_dns_servers, var.private_dns_servers))}"
...
#### Create the DNS VM
##################################
resource "vsphere_virtual_machine" "dns_ds_cluster" {
  #depends_on = ["vsphere_folder.ocpenv"]
  folder = "${var.folder_path}"
  ...
  # VM Specifications
  ####
  count            = "${var.datastore_cluster_id != "" ? var.dns["nodes"] : 0}"
  resource_pool_id = "${var.vsphere_resource_pool_id}"
